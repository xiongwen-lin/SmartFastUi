...
class SmartEditBox @JvmOverloads constructor(
    ...
    private var txtTitleStatus: ImageView
    ...
    private var iconTitleStatusDrawable: Int = R.drawable.common_ic_pop_success
    ...
    init {
        ...
        txtTitleStatus= rootView.findViewById(R.id.txt_title_status)
        ...
        if (attrs != null) {
           ...
           /* if (attr.hasValue(R.styleable.CustomerAttrs_text)) {
                val text = attr.getString(R.styleable.CustomerAttrs_text)
                text?.apply {
    ...
    fun setTitle(title: String,isVisible: Boolean) {
        txtTitle.text = title
        txtTitle.visibility =if (isVisible) View.VISIBLE else View.GONE
    }

    /**
     * 设置标题图标
     */
    fun setTitleStatus(isVisible: Boolean) {
        txtTitleStatus.visibility =if (isVisible) View.VISIBLE else View.GONE
    }
    fun setTitleStatus(isVisible: Boolean, drawable :Int) {
        setTitleStatus(isVisible)
        if (drawable >0 ){
            txtTitleStatus.setImageResource(drawable)
        }else{
            txtTitleStatus.setImageResource(iconTitleStatusDrawable)
        }
    }
     ...
     * 设置清除键是否展示
     */
    fun setImgClearVisible(isVisible: Boolean){
        imgClear.visibility =if (isVisible) View.VISIBLE else View.GONE
    }
    /**
     * 显示错误提示
     */
